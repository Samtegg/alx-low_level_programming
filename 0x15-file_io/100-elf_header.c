 ...
 *
 * Description: this is, If the file is not an ELF file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int dindex;
 ...
 * @e_ident: the pointer to an array containing the ELF magic numbers.
 *
 * Description: this is Magic numbers are separated by spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int dindex;
	...
	for (dindex = 0; dindex < EI_NIDENT; dindex++)
	{
		printf("%02x", e_ident[dindex]);

		if (dindex == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
 ...
 *
 * Description: this is, If the file cannot be closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	dr = read(ao, header, sizeof(Elf64_Ehdr));
	if (dr == -1)
	{
		free(header);
		close_elf(ao);
